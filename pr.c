...
int main(int argc, char **argv) {
  ...
  //Setup
  char *srcIP[5] = {"10.0.0.107","10.0.0.108","10.0.0.109","10.0.0.110","10.0.0.111"};
  char *dstIP = "85.13.131.154";
  int dstPort[5] = {670,85,82,80,84};
  int srcPort[5] = {80,80,80,80,80};
  ...
  addr_in.sin_port = htons(dstPort[i]);
  ...
  ipHdr->saddr = inet_addr(srcIP[i]); //32 bit format of source address
  ...
  //Populate tcpHdr
  tcpHdr->source = htons(srcPort[i]); //16 bit in nbp format of source port
  tcpHdr->dest = htons(dstPort[i]); //16 bit in nbp format of destination port
  ...
  pTCPPacket.srcAddr = inet_addr(srcIP[i]); //32 bit format of source address
